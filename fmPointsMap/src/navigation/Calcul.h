 ...
 */
  /*static double distance_circle(double current_angle,
  ...
  */
  static Destination turning_aim(double current_angle,
				  Point current_position,
				  Destination d1)
  {
    double angle_to_dest = angle(current_position, d1);
    ...
    if (fabs(angle_diff) >= 180)
      (angle_diff > 0) ? d2.set_id(_COUNTERWISE_) : d2.set_id(_CLOCKWISE_);
    else
      (angle_diff >= 0) ? d2.set_id(_CLOCKWISE_) : d2.set_id(_COUNTERWISE_);
    return d2;
  }
};
