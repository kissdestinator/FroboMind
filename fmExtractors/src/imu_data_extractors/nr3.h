...
public:
	...
	NRmatrix<T> product(const NRmatrix<T>& A, const NRmatrix<T>& B);
	NRvector<T> product(const NRmatrix<T>& A, const NRvector<T>& b);
	double product(const NRvector<T>& A, const NRvector<T>& b);
	NRvector<T> vector_subtract(const NRvector<T>& a, const NRvector<T>& b);

	void resize(int newn, int newm); // resize (contents not preserved)
	void assign(int newn, int newm, const T &a); // resize and assign a constant value
	~NRmatrix();
	//NRmatrix<T> product(const NRmatrix<T>& A, const NRvector<T>& b);
...
template<class T> double product(const NRvector<T>& A, const NRvector<T>& b) {
	double v = 0;
