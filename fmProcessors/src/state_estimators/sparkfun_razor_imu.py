...
class razorIMU():
	...
	def __init__(self):
		...
		self.pub = rospy.Publisher('razorIMU', Imu)
	...
	def gyro_callback(self, data):
		if self.gyro:
			self.dt = data.header.stamp.to_sec() - self.gyro.header.stamp.to_sec()
		self.gyro = data
		...
		if self.gyro_calibrated:
			...
			if self.accel_calibrated:
				self.roll = (1-self.filter)*(self.roll + self.gyro.x*self.dt) + self.filter*self.accel.y
				self.pitch = (1-self.filter)*(self.pitch + self.gyro.y*self.dt) + self.filter*self.accel.z
				self.yaw = (1-self.filter)*(self.yaw + self.gyro.z*self.dt) + self.filter*self.accel.x
