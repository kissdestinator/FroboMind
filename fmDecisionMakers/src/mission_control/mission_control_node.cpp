...
int main(int argc, char **argv) {
	...
	n.param<std::string> ("P_Filter_Sub_Top", mc.p_filter_sub_top, "/");
	...
	mc.map_sub = nh.subscribe<fmMsgs::Vector3>(mc.map_sub_top.c_str(),1,&MISSION_CONTROL::map_callback, &mc);
	mc.p_filter_sub = nh.subscribe<fmMsgs::Vector3>(mc.p_filter_sub_top.c_str(),1,&MISSION_CONTROL::p_filter_callback, &mc);
